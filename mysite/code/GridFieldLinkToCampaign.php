 ...
 */
class GridFieldLinkToCampaign implements GridField_HTMLProvider, GridField_ActionProvider, GridField_URLHandler
{
     ...
     */
    public function __construct($actionName, $buttonText, $targetFragment = 'before', $buttonConfig = array())
    {
    ...
    public function getHTMLFragments($gridField){
		...
		$dropdownField = new DropdownField(
			'GroupID', 
			false, 
			ProspectCampaign::get()->filter(array('MemberID' => Member::currentUserID()))->map('ID', 'Title')
		);
		$dropdownField->setEmptyString('(Select a campaign)');
		$dropdownField->addExtraClass('no-change-track');
		$dropdownField->setAttribute('style', 'margin-left:0;max-width:256px;padding:6px;');
		...
		$forTemplate->Fields->push($dropdownField);
		$forTemplate->Fields->push($button);

        return array(
            $this->targetFragment => $forTemplate->renderWith(__CLASS__)
        );
    }
